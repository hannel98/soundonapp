...
async def resolve_user_from_authorization(authorization: Optional[str]) -> dict:
    ...
    # Try as session_token (Emergent Google)
    ...
    if existing:
        ...
        await db.users.update_one(
            ...
            {"$set": {
                "providers": providers,
                "google_id": google_id,
                "avatar_url": existing.get("avatar_url") or google_picture,
                "display_name": existing.get("display_name") or google_name,
                "updated_at": now_utc(),
            }},
        )
    else:
...
async def list_tracks():
    docs = await db.tracks.find({}, {"_id": 0}).to_list(200)
    return [Track(**d) for d in docs]


@api_router.get("/beats", response_model=List[BeatModel])
async def list_beats():
    docs = await db.beats.find({}, {"_id": 0}).to_list(200)
    return [BeatModel(**d) for d in docs]


@api_router.get("/videos", response_model=List[Video])
async def list_videos():
    docs = await db.videos.find({}, {"_id": 0}).to_list(200)
...
SEED_NEWS = [
    {"id": "n1", "title": "AI Music Generation Transforms the Industry", "summary": "AI tools like Sound enable anyone to produce professional-quality tracks.", "body": "Artificial intelligence is revolutionizing how music is created, with tools like Sound enabling anyone to produce professional-quality tracks. The democratization of music production opens new opportunities for aspiring artists worldwide.", "category": "AI", "image_url": "https://images.pexels.com/photos/2607311/pexels-photo-2607311.jpeg?auto=compress&cs=tinysrgb&w=800", "published_at": "2026-01-28"},
